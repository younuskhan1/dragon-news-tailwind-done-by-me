...
const LeftSideNews = ({categoriesNames}) => {
    ...
    return (
        <div className="w-[100%] lg:w-[25%] mb-5">
            <h1 className="text-center text-black text-xl font-semibold">All Categories</h1>
            <ul className='pt-8'>
                {categoriesNames.map((categoryName) => <li className='py-2 px-4 hover:bg-slate-200 rounded-sm text-center mb-3 cursor-pointer text-[#9f9f9f] hover:text-[#403f3f] font-bold transition-all ease-in duration-300' key={categoryName.id}>{categoryName.name}</li> )}
            </ul>
            <div>
                <div className='w-full mt-4'>
                    <img className='w-full' src={cuteGirl} alt="cuteGirl" />
                </div>
                <p className='mt-4 leading-7 [word-spacing:2px]'>Bayern Slams Authorities Over Flight Delay to Club World Cup</p>
            </div>
